...
impl DocRouter {
    pub fn new() -> Self {
        Self {
            client: Client::new(),
            cache: DocCache::new(),
            summarizer: AiSummarizer::new().unwrap(),
        }
    }
    ...
    async fn lookup_item(
        ...
        for item_type in item_types.iter() {
            let file = if item_type.is_empty() {
                format!("{item_name}/index.html")
            } else {
                format!("{item_type}.{item_name}.html")
            };
            ...
            let url = if let Some(ver) = version.clone() {
                ...
                format!(
                    "https://docs.rs/{}/latest/{}/{file}",
                    crate_name, crate_name
                )
            } else {
                format!(
                    "https://docs.rs/{}/latest/{}/{}/{file}",
                    crate_name, crate_name, module_path
                )
            };
            ...
            if response.status().is_success() {
                ...
                let result_text = self
                    .summarizer
                    .summarize_docs_with_ai(&crate_name, &markdown_body)
                    .await
                    .unwrap();
...
impl mcp_server::Router for DocRouter {
    ...
    fn instructions(&self) -> String {
        ...
        for better compatibility with language models."
            .to_string()
    }
    ...
    fn list_tools(&self) -> Vec<Tool> {
        vec![
            ...
            Tool::new(
                ...
                "Look up documentation for a specific item in a Rust crate (returns markdown)"
                    .to_string(),
                json!({
                    "type": "object",
                    "properties": {
            ...
            match tool_name.as_str() {
                "lookup_crate" => {
                    let crate_name = arguments
                        ...
                        .and_then(|v| v.as_str())
                        .ok_or_else(|| {
                            ToolError::InvalidParameters("crate_name is required".to_string())
                        })?
                        .to_string();
                ...
                "search_crates" => {
                    let query = arguments
                        ...
                        .and_then(|v| v.as_str())
                        .ok_or_else(|| {
                            ToolError::InvalidParameters("query is required".to_string())
                        })?
                        .to_string();
                ...
                "lookup_item" => {
                    ...
                    let item_path = arguments
                        ...
                        .and_then(|v| v.as_str())
                        .ok_or_else(|| {
                            ToolError::InvalidParameters("item_path is required".to_string())
                        })?
                        .to_string();
    ...
    fn read_resource(
        ...
        Box::pin(async move { Err(ResourceError::NotFound("Resource not found".to_string())) })
    }
